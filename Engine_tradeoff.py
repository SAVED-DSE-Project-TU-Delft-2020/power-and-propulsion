...
#matrix= np.array([[0.3088,0.2897,0.3867,0.1922],[0.2163,0.3458,0.1755,0.6288],[0.4509,0.2473,0.1194,0.0575],[0.0240,0.1172,0.3184,0.1215]])
#weights = np.array([0.3277,0.3058,0.2876,0.0790])
...
#Engine and Propeller selecion

#matrix with normalized noise, cost and efficiencies
#weights for these trade of criteria
...
def normalizing():
    ...
    #Noise
    noise_matrix = np.array([n_1,n_2,n_3])
    max_noise = 107.58  #dummy values
    min_noise = 103.61
    
    noise_list = []
    for i in range(len(noise_matrix)):
        n_noise = (noise_matrix[i]-min_noise)/(max_noise-min_noise)
        noise_list.append(1-n_noise)
    #Cost
    cost_matrix = np.array([c_1,c_2,c_3])
    max_cost = 195
    min_cost = 148
    
    cost_list = []
    for j in range(len(cost_matrix)):
        n_cost = (cost_matrix[j] - min_cost)/(max_cost-min_cost)
        cost_list.append(1-n_cost)
        
    #Weight
    weight_matrix = np.array([w_1,w_2,w_3])
    max_weight = 360
    min_weight = 250
    
    weight_list = []
    for l in range(len(weight_matrix)):
        n_weight = (weight_matrix[l] - min_weight)/(max_weight-min_weight)
        weight_list.append(1-n_weight)
        
    #Efficiency
    efficiency_matrix = [e_1,e_2,e_3]
    max_efficiency = 0.8
    min_efficiency = 0.6
    
    efficiency_list = []
    for k in range(len(efficiency_matrix)):
            n_efficiency = (efficiency_matrix[k] - min_efficiency)/(max_efficiency- min_efficiency)
            efficiency_list.append(n_efficiency)
    
    engine_1 = [noise_list[0], cost_list[0], weight_list[0], efficiency_list[0]]
    engine_2 = [noise_list[1], cost_list[1], weight_list[1], efficiency_list[1]]
    engine_3 = [noise_list[2], cost_list[2], weight_list[2], efficiency_list[2]]
    
        
            
    return np.array([engine_1, engine_2, engine_3])
    
#Noise =0.3, Cost = 0.1, Weight = 0.2, Efficiency = 0.4    
